from pylatex import Document, Command, UnsafeCommand, Tabular,TextColor
...
class Resume3:
    def __init__(self, name, email, number):
        borders = {"tmargin": "0.8in", "rmargin": "0.65in", "lmargin": "0.65in", "bmargin": "0.8in"}
        self.doc = Document(documentclass="resume3", geometry_options=borders)
    ...
    def addCustomCommands(self):
        ...
        BoldHeading1 = UnsafeCommand('newcommand', r'\BoldHeading', options=1,
                                    extra_arguments=r"""
                                   {\bf{#1}\linebreak}
                               """)
    ...
    def addTitle(self, student_name, email, number):
        ...
        self.doc.preamble.append(Command("address", f"Phone: {number}   Email: {email}"))
    ...
    def addExperience(self, jobs):
        self.doc.append(Command("begin", ["rSection", "Job Experience:"]))
        for job in jobs:
    ...
    def addRelevantCoursework(self, classes):
        self.doc.append(Command("begin", ["rSection", "Coursework Experience:"]))
...
def main():
    resume = Resume3("Ariel Chen", "achen@calpoly.edu", "925-786-3014")
    ...
    resume.doc.generate_tex("./resume3")
    resume.doc.generate_pdf("testpdf3", compiler="/Library/TeX/texbin/pdflatex")
